---
name: dafu-subs
description: 烤肉视频字幕工作流：当处理视频字幕时使用，包括在线语音识别、源语言 SRT 生成与清理，以及按视频语境翻译为简体中文字幕。
---

# 大福烤肉字幕流程

这份 skill 用于处理视频字幕工作流，包括视频素材下载、音频提取、封面保存、第三方API ASR、按视频语境翻译为简体中文字幕、烧制字幕，以及检查总结。

适用场景包括游戏实况、主播解说等。除非用户明确指定只处理某一步，否则优先按“先保留源语言字幕，再按需翻译”的原则执行。
...
- 只有以下两种情况需要执行本地配置：
  1. 检查 `config/dafu-subs.local.json` 时发现文件不存在、字段缺失或字段为空。
...
- 触发本地配置时，必须按以下顺序逐项询问用户，一次只问一个问题，等待用户回答并确认后再进入下一项：
  ...
  3. `subtitle_font_style`：询问字幕字体样式路径。
- 三项默认值如下；询问用户时必须展示当前项对应的默认值，并让用户明确确认使用默认值或输入新值：
  ...
  ```
- 本地配置完成后：
  1. `api_key_source` 对应 `tools/api_volcengine_asr.py` 中的 `HARDCODED_API_KEY` 变量。
  2. `video_domain_context` 作为第 3 步“翻译源 ASR 字幕”的领域语境规则。
  3. `subtitle_font_style` 作为第 4 步“烧制流程”的字体样式规则。
- 后续标准流程直接使用这些已固化内容，不在每一步重复读取 `config/dafu-subs.local.json`。
...
1. **预处理与下载**
   ...
   - 语言代码统一使用以下映射：`<source>` 为以下之一：中文普通话 `zh-CN`，英语 `en-US`，日语 `ja-JP`，韩语 `ko-KR`，葡萄牙语 `pt-BR`。
   - 如果无法可靠判断，先向用户确认。
...
2. **调用火山引擎 API**
   ...
   - 首次使用或触发 `本地配置` 时，必须先把 `api_key_source` 写入 `tools/api_volcengine_asr.py` 的 `HARDCODED_API_KEY`，并把领域语境和字幕样式写入当前 skill 流程规则。
   ...
   - 火山引擎 ASR 是必需联网步骤；在当前 Codex 沙盒环境中，直接调用经常会因网络限制出现 DNS 错误 `nodename nor servname provided, or not known`，这不是火山脚本问题。
   ...
   - 推荐调用方式：`UV_CACHE_DIR="./.uv-cache" HARDCODED_AUDIO_FILE="<音频路径>" HARDCODED_AUDIO_LANGUAGE="<source>" HARDCODED_OUTPUT_JSON="<原音频文件名>.volc-asr-<source>.json" HARDCODED_OUTPUT_SRT="<原音频文件名>.volc-asr-<source>.srt" uv run ./tools/api_volcengine_asr.py`。
...
3. **翻译源ASR 字幕**
   ...
   - 将源语言 SRT 逐条翻译成简体中文，只使用当前字幕块文本，不参考或复用已有中文字幕。
   - 必须保留原 SRT 的编号、时间轴、空行结构，确保中文块与源字幕同编号一一对应。
   - 翻译时优先理解视频领域语境，而不是只做字面机翻或固定术语替换。
   - 《哈利波特：魔法觉醒》视频应优先采用游戏常用术语，如角色名、卡牌名、咒语名、回响、伙伴卡、召唤物、段位、决斗表达等。
...
4. **烧制流程**
   ...
   - 字体样式统一放在 `./fonts/`，默认使用：`./fonts/subtitle_font_style_default.json`。
   ...
   - 硬烧使用 `ffmpeg -vf "ass=filename='<bilingual.ass>'" -c:v libx264 -preset medium -crf 18 -c:a copy`。

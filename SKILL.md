...
4. **烧制流程**
   ...
   - 默认采用四档事件级位置，按换行后的实际行数选择：
     - `1中 / 1源`：中文 `y = PlayResY - 56`，源语言 `y = PlayResY - 10`。
     - `2中+ / 1源`：中文 `y = PlayResY - 70`，源语言 `y = PlayResY - 14`。
     - `1中 / 2源+`：中文 `y = PlayResY - 86`，源语言 `y = PlayResY - 18`。
     - `2中+ / 2源+`：中文 `y = PlayResY - 100`，源语言 `y = PlayResY - 12`。
